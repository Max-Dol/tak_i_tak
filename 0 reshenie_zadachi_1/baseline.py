...
def merge_data(train, resnet, attributes, text_and_bert):
    ...
    # Объединение данных с аттрибутами
...
def combine_embeddings(row):
    ...
    # Объединение эмбеддингов текста из столбцов 'bert_embeddings_1' и 'bert_embeddings_2'
    text_embeddings = np.concatenate([row['bert_embeddings_1'], row['bert_embeddings_2']])
...
def train_model(X_train, y_train):
    ...
    joblib.dump(model, 'baseline2.pkl')
    return model
...
def main():
    ...
    attributes = process_attributes(attributes) # process_attributes
